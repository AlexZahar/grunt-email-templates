...
For the contribute.to email campaign, we are using the Mailgun API.  
The final email templates models can be found in the "./email_templates".
They have been designed using the grunt workflow. The handlebars templates are compiled together with the SCSS, resulting in a HTML file with inlined CSS.
For more details on how to set up the grunt workflow, follow the steps described underneath.

Our current template names used within mailgun:
...
For backend integration, find the node.js code snippets inside each of the email template folder "/email_templates/..". Each template has it's own variables.

To send new test emails using Mailgun, the html code that has been generated by the grunt workflow, has to be copy-pasted into a new email template within the Mailgun platform. There you can see a preview of the email.
...
We encourage you **not** to store sensitive data in your git repository. If you must, please look into [git-encrypt](https://github.com/shadowhand/git-encrypt) or some other method of encrypting your configuration secrets.
